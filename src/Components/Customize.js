...
class Customize extends Component {
  ...
  goToOrder() {
    ...
    flavorsIds.map((e) =>
      axios
        .post("/api/bag_list", { bag_id: orderId, flavor_id: e.id })
        .then((res) => {})
    );
  ...
  checkFlavors = (copiedArray) => {
    if (copiedArray.length === 0) {
      ...
      if (
        copiedArray.some((e) => e === "Vanilla") &&
        copiedArray.some((e) => e === "Chocolate")
      ) {
        this.setState({ pic: vanillachocolate });
      } else if (
        copiedArray.some((e) => e === "Vanilla") &&
        copiedArray.some((e) => e === "Strawberry")
      ) {
        this.setState({ pic: vanillastrawberry });
      } else if (
        copiedArray.some((e) => e === "Vanilla") &&
        copiedArray.some((e) => e === "Carmel")
      ) {
  ...
  deleteFlavor(e) {
    ...
    this.setState({ price: this.state.price - 1.0 });
    ...
    return (
      <div className="container">
        ...
        <div className="flavor-wrapper">
          <label>Flavors</label>
          <button className="appBtn" value="Vanilla" onClick={this.addFlavor}>
            Vanilla
          </button>
          <button className="appBtn" value="Chocolate" onClick={this.addFlavor}>
            Chocolate
          </button>
          <button
            className="appBtn"
            value="Strawberry"
            onClick={this.addFlavor}
          >
            Strawberry
          </button>
          <button className="appBtn" value="Carmel" onClick={this.addFlavor}>
            Carmel
          </button>
          <label>AddIns</label>
          <button
          ...
          <button className="appBtn" value="Brownies" onClick={this.addAddIn}>
            Brownies
          </button>
          <button className="appBtn" value="Sprinkles" onClick={this.addAddIn}>
            Sprinkles
          </button>
        </div>
        <div className="created-flavor">
          <div>
            ...
            <input
              ...
              onChange={(e) => this.setState({ flavorName: e.target.value })}
            />
          </div>
          <div>
            <h2>Flavors:</h2>{" "}
            {this.state.flavors.map((e, i) => (
              <button
                className="appBtn"
                key={i}
                value={e}
                onClick={() => this.deleteFlavor(e)}
              >
                {e}
              </button>
            ))}
          ...
          <div>
            <h2>AddIns:</h2>{" "}
            {this.state.addIns.map((e, i) => (
              <button
                className="appBtn"
                key={i}
                value={e}
                onClick={() => this.deleteAddIn(e)}
              >
                {e}
              </button>
            ))}
          ...
          <button className="appBtn" onClick={() => this.submitFlavor()}>
            Submit Flavor
          </button>
          <button className="appBtn" onClick={() => this.goToOrder()}>
            Order
          </button>
        </div>
      </div>
...
export default connect(mapStateToProps, { addNewFlavor, addToTotal })(
  Customize
);
